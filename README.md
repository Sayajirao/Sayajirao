<div align="center">

# Hello There 👋  
I'm **Sayajirao Danke**  

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=0&color=2E9EF7&center=true&vCenter=true&width=600&duration=2000&lines=DevOps+Engineer;1.10%2BYears+Hands-on+Experience;AWS+%7C+ECS+%7C+EKS+%7C+Terraform;CI%2FCD+%7C+Automation+%7C+Cost+Optimization)](https://git.io/typing-svg)

</div>

---

## 🚀 About Me

I’m an **DevOps Engineer** with **1.10 years of hands-on experience** designing, automating, and operating **production-grade, scalable, and secure cloud infrastructure**.

I specialize in **AWS, Terraform, CI/CD automation, container orchestration (ECS & EKS), and cloud cost optimization**, with proven impact across **multiple client environments and production workloads**.

- 🔭 Managing **AWS production infrastructure (5+ environments)**
- ⚙️ Automating infrastructure using **Terraform & CloudFormation**
- 🚀 Building **CI/CD pipelines** with GitLab CI, GitHub Actions & CodePipeline
- 📉 Driving **cost optimization & reliability (saved $250K+ annually)**
- 🤝 Mentoring interns & leading DevOps best practices

---

## 🛠️ Tech Stack

### **Cloud Platform**
<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" width="40"/>
</p>

---

### **Containerization & Orchestration**
<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original-wordmark.svg" width="40"/>
</p>

- Docker  
- AWS ECS / Fargate  
- AWS EKS (IRSA, PV/PVC, ConfigMaps, Secrets)

---

### **Infrastructure as Code**
<p align="left">
  <img src="https://www.vectorlogo.zone/logos/terraformio/terraformio-icon.svg" width="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" width="40"/>
</p>

- Terraform (Modular IaC – 15+ reusable modules)
- AWS CloudFormation

---

### **CI/CD & Automation**
<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/github/github-original.svg" width="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/gitlab/gitlab-original.svg" width="40"/>
  <img src="https://www.vectorlogo.zone/logos/amazon_aws/amazon_aws-icon.svg" width="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/circleci/circleci-plain.svg" width="40"/>
</p>

- GitHub Actions  
- GitLab CI/CD  
- AWS CodePipeline  
- CircleCI  
- Python & Bash scripting


---

### **Monitoring, Security & Observability**
<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" width="45"/>
</p>

- AWS CloudWatch (Custom Metrics, Alarms, SNS)
- IAM, RBAC, IRSA
- AWS WAF, Secrets Manager
- VPC Flow Logs, SSL/TLS


---

### **Databases**
<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/postgresql/postgresql-original-wordmark.svg" width="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg" width="40"/>
</p>

- RDS MySQL & PostgreSQL
- DocumentDB
- DynamoDB
- Qdrant (Vector DB on EKS)

---

## 🏢 Professional Experience

### **DevOps Engineer — CloudShastra Infotech Solutions**
📍 Miraj, India | **Mar 2024 – Present**

- Managing **AWS infrastructure across 5+ production environments** with **99.9% uptime**
- Reduced **incident response time by 60%** using automated monitoring & alerting
- Designed & led **internship program (10+ interns)** with 90% conversion rate
- Achieved **40% cloud cost reduction**, saving **$50K+ annually**
- Built **5+ Python/Bash automation scripts**, eliminating **15 hrs/week manual work**
- Reduced **deployment errors by 85%** through CI/CD & automation

---

### **Client: AI-Powered Lead Generation Platform (GenAI)**

- Architected AWS infra for **1K+ concurrent users**
- ECS (20+ containers), Lambda (3+ functions), SageMaker & Bedrock
- Reduced cost from **$77K → $20K/month (80% savings)**
- Built GitLab CI/CD pipelines enabling **30+ deployments/week**
- Configured NGINX handling **600+ RPS with 99.95% uptime**
- Optimized Docker images (**2GB → 800MB**)
- Deployed **Qdrant Vector DB on EKS**
- Implemented Terraform IaC cutting provisioning time by **92%**

---

### **Client: Enterprise Fin-Tech Web Application**

- Designed **multi-AZ AWS infrastructure** with ECS, RDS & CloudFront
- Auto-scaled ECS EC2 from **2 → 50 instances**
- Reduced deployment time **2 hrs → 15 mins**
- Blocked **1K+ malicious requests/month** using AWS WAF
- Built **12 reusable Terraform modules**
- Reduced page load time by **50%**

---

### **Client: Multi-Cloud Modernization**

- Migrated **8 applications from ARUBA → AWS** with zero downtime
- Saved **$252K annually** using Reserved Instances & Savings Plans
- Automated CI/CD across **GitLab, GitHub Actions & CircleCI**
- Migrated **10+ Bitbucket repos to GitHub** (100% history preserved)
- Automated SSL for **10+ domains**

---

## 📜 Certifications

- 🏅 **AWS Certified Cloud Practitioner**
- 🏅 **AWS Solutions Architect – Associate (in progress)**

---

## 🌐 Connect With Me

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/sayajiraodanke)
[![Email](https://img.shields.io/badge/Email-Contact-red?style=for-the-badge&logo=gmail)](mailto:sayajiraodanke1412.official@gmail.com)

</div>

---

<div align="center">

### 💡 *"Automating today, optimizing costs tomorrow."*

</div>
